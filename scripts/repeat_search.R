...
repeat_search <- function(ss5,ss3, ss5_seq, ss3_seq, mlim_km = 3, lim_km = 14, idx){
  ...
  kmer_search <- lapply(seq(mlim_km,lim_km), function(klen){
    ...
    #  matchkmers <- matchPDict(kmers_seq ,ss3,  ) # Search in the other splice site (inverse)
    ...
    if(nrow(matchkmers) >= 1){
      matchkmers <- matchkmers %>% 
        rowwise() %>% 
        mutate(ss_match_5 = match_ss(start.ss5 , end.ss5, "5ss"), 
...
repeat_ss <- parallel::mcmapply(repeat_search, ss_l_seq, ss_r_seq, ss2_l, ss2_r, 3,20, 
                                seq_along(ss_l_seq), SIMPLIFY = F, mc.cores = 32)
...
all_introns <- data.frame(event_coord  =  as.character(intron.gr), 
                          ss5_seq = as.character(ss2_l), 
                          ss3_seq = as.character(ss2_r))
#longest_repeats <- fread("output/candidates_direct_repeats_full_v2.tab")
...
fwrite(repeats_match_ss, "output/candidates_longest_repeats_v2.tab")
fwrite(repeat_ss, "output/candidates_direct_repeats_full_v2.tab")
